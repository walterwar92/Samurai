...
class MpsTelemetryPoint(BaseModel):
    ...
    x_local: float | None = Field(
        default=None,
        description='Позиция робота (м) в локальном фрейме старта; X-ось '
                    'локального фрейма направлена вдоль курса робота на момент '
                    '_on_scenario_run. None для старой телеметрии.',
    )
    y_local: float | None = Field(
        default=None,
        description='Позиция робота (м) в локальном фрейме старта; Y-ось '
                    'локального фрейма — налево от X (правая система координат). '
