...
2. **Каноническая `ė_int = v_target − v` делает модель неуправляемой.**
   ...
   неуправляемая мода на λ=1 (ctrb rank 4/5). DARE для такой модели
   не решается. **Решение (выбор пользователя): переопределить
   `ė_int = θ_ref − θ`** (`θ_ref ≡ 0` ⇒ `ė_int = −θ`) — интеграл ошибки
   **курса**, `A_c[4][2]=−1` вместо `A_c[4][1]=−1`. Модель становится
   полностью управляемой (rank 5/5), DARE решается, замкнутый контур
   строго устойчив (проверено симуляцией: max|λ| ≈ 0.991, сценарий
   `reached`). Вариант «∫ ошибки позиции» (`A_c[4][0]=−1`) тоже
   управляем, но потребовал бы фидфорвард `x_ref[4]=−∫s_ref dt` в раннере
   (`mps_runner` + `mps_node`) — иначе регулятор тянет `e_int→0` и
   откатывает робота назад. Вариант с курсом работает без изменений
   логики референса, т.к. `θ_ref ≡ 0` (экзогенного члена нет).
...
| 5 | Состояние `e_int` (выявлено при реализации) | **`ė_int = −θ`** (∫ ошибки курса, `A_c[4][2]=−1`). Старое `ė_int = −v` делало модель неуправляемой; вариант ∫позиции потребовал бы изменений логики референса в раннере. |
...
ė_int = −θ                (θ_ref ≡ 0 — интеграл ошибки курса)
```

`A_c`: `[0][1]=1, [1][1]=−1/τ_v, [2][3]=1, [3][3]=−1/τ_ω, [4][2]=−1`, остальное 0.
`B_c`: `[1][0]=1/τ_v, [3][1]=1/τ_ω`, остальное 0.

> **Решение #5 (см. §2.6):** `ė_int = −θ` (интеграл ошибки **курса**),
> т.е. `A_c[4][2]=−1`. Прежний вариант `ė_int = v_target − v` (`A_c[4][1]=−1`)
> делал модель неуправляемой (`s + e_int = const`). С `A_c[4][2]=−1` модель
> полностью управляема, DARE решается, замкнутый контур строго устойчив, и
> `x_ref = [s_ref, v_target, 0, 0, 0]` остаётся валидным без фидфорварда
> (`θ_ref ≡ 0`).
...
- `config.yaml` — секция `mps:` (непрерывная каноническая `A_c[4][2]=−1` + `mps.plant`)
- `compute_node/frontend/src/components/mps/OdeCard.tsx` — подпись Ts + уравнение `ė_int`
- `compute_node/frontend/src/lib/mps/canonical.ts` — `CANONICAL_PATTERN_A` ячейка `e_int`: col 1→2
- `compute_node/frontend/src/lib/mps/tokenMap.ts` — токен `coef_eint_v` → `coef_eint_theta` (A[4][2])
- `docs/mps/api.md` — контракт
...
> → добавлен корректирующий этап «`e_int` → ∫ ошибки курса + фикс
