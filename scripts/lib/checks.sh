...
# Гарантирует listener 1883 + anti-freeze limits + (опц.) auth.
# Использование: check_mosquitto [--auth user pass]
check_mosquitto() {
...
MQTTCFG
    ...
    if $with_auth; then
        ...
        desired=$(echo -e "$desired" | sed 's/allow_anonymous true/allow_anonymous false/')
        desired="${desired}
